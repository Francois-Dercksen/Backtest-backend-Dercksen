...
Leg-level metrics (compute_leg_metrics) return TWO independent metric sets:
  ...
  "contribution": measures return_contribution directly (fraction of $1 TOTAL
    equity) -- how much this leg moved the Net Portfolio's wealth. NOT divided
    by capital_weight: options legs spend ~100% of allocated capital on
    premium every period, so dividing by capital_weight would produce
    leg_return = -1.0 (-100%) in any non-exercised period, and cumprod() would
    permanently zero the series the first time that happens.

  "strategy": measures return_contribution / notional -- the strategy's own
    standalone performance, as if it were its own investment. For Long/Short
    Portfolio, notional == the leg's weight, so this recovers the portfolio's
    raw return exactly. For options legs, notional is the position size (not
    the cash spent), so this stays bounded and meaningful.
...
#   notional             -- the position size for that period
# plus leg-specific diagnostic columns for reporting.
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Leg-level metrics: contribution basis + strategy (standalone) basis
# ─────────────────────────────────────────────────────────────────────────────

def compute_leg_metrics(leg_df, frequency, risk_frequency, rolling_window, ppy):
    """
    Returns a dict with two independent metric sets for a leg:

      "contribution": measures return_contribution directly (fraction of $1
        TOTAL equity) -- how much this leg moved the Net Portfolio's wealth.
        NOT divided by capital_weight: options legs spend ~100% of allocated
        capital on premium every period, so dividing by capital_weight would
        produce leg_return = -1.0 (-100%) in any non-exercised period, and
        cumprod() would permanently zero the series the first time that
        happens. This basis avoids that entirely.

      "strategy": measures return_contribution / notional -- the strategy's
        own standalone performance, as if it were its own investment. For
        Long/Short Portfolio, notional == the leg's weight, so this recovers
        the portfolio's raw return exactly. For options legs, notional is the
        position size (not the cash spent), so this stays bounded and
        meaningful instead of blowing up like a capital_weight-based ratio
        would.
    """
    df = leg_df.copy()
    df["leg_return"] = df["return_contribution"]
    contribution_metrics = compute_metrics(df, "leg_return", frequency, risk_frequency, rolling_window, ppy)

    if "notional" in df.columns and df["notional"].abs().sum() > 0:
        df["strategy_return"] = np.where(
            df["notional"] != 0,
            df["return_contribution"] / df["notional"],
            0.0,
        )
    else:
        df["strategy_return"] = df["return_contribution"]
    strategy_metrics = compute_metrics(df, "strategy_return", frequency, risk_frequency, rolling_window, ppy)
    ...
    Returns a dict:
      {
        ...
        "legs": {
          leg_key: {
            "results_df": ...,
            "metrics": {"contribution": {...}, "strategy": {...}, "option_stats": {...}?},
            "label": ..., "type": ...,
          }
        },
      }
